...
const COMMAND_LIST_ALL_NODE = 0x13,
...
lightify.prototype.connect = function() {
  ...
  return new Promise(function(resolve, reject) {
    ...
    self.connectTimeout = setTimeout(function() {
      ...
      self.logger &&
        self.logger.debug("can not connect to lightify bridge, timeout");
      self.client.destroy();
    }, 20000);
    self.client.on("data", function(data) {
      self.logger &&
        self.logger.debug("socket data: [%s]", data.toString("hex"));
      ...
      if (expectedLen > data.length) {
        ...
        self.readBuffer = Buffer.alloc(data.slice(data.length - expectedLen));
      }
      self.onData(data);
    });
...
lightify.prototype.setDisconnectTimer = function() {
  ...
  var _setTimer = function() {
    ...
    self.disconnectTimer = setTimeout(function() {
      ...
      self.logger &&
        self.logger.debug("setDisconnectTimer: client.end() called");
...
lightify.prototype.sendNextRequestAutoClose = function(buffer) {
    ...
    switch (self.client.connected) {
      ...
      case true:
        if (self.buffers.length > 0) {
          ...
          self.logger &&
            self.logger.debug(
              "sendNextRequest: sent buffer done, remaining length=" +
                self.buffers.length
            );
      ...
      case undefined:
        ...
        self.connectEx(function() {
          self.logger &&
            self.logger.debug(
              "sendNextRequest: connected! buffers.length=" +
                self.buffers.length
            );
          checkSend();
        });
  ...
  if (buffer) {
    ...
    self.logger &&
      self.logger.debug(
        "sendNextRequest: push(buffer) length=" + (this.buffers.length + 1)
      );
...
lightify.prototype.connectEx = function(cb, errorCb) {
  ...
  if (!this.client) {
    // || this.client.destroyed || !this.client.readable) {
    return this.connect()
      .then(func)
      .catch(function(error) {
        if (self.connectErrorCount++ < 5)
          setTimeout(function() {
          ...
          errorCb &&
            errorCb("Can not connect to Lightify Gateway " + self.ip, error);
          self.logger &&
            self.logger.debug("Can not connect to Lightify Gateway " + self.ip);
        }
      });
  }
...
lightify.prototype.sendCommand = function(cmdId, body, flag, cb, packageSize) {
  ...
  return new Promise(function(resolve, reject) {
    var buffer = Buffer.alloc(8 + body.length);
    ...
    buffer.writeUInt32LE(++self.seq, 4); // request id
    body.copy(buffer, 8);
    var cmd = {
      seq: self.seq,
      cmdId: cmdId,
...
lightify.prototype.discoverZone = function() {
  return this.sendCommand(COMMAND_LIST_ALL_ZONE, Buffer.from([0x0]), function(
    data,
    pos
  ) {
...
lightify.prototype.nodeBrightness = function(
  mac,
  brightness,
  stepTime,
  isGroup
...
lightify.prototype.nodeTemperature = function(
  mac,
  temperature,
  stepTime,
  isGroup
...
lightify.prototype.nodeColor = function(
  mac,
  red,
  green,
  blue,
...
var exports = (module.exports = {
  ...
  isBrightnessSupported: function(type) {
    return (
      getNodeType(type) === tf.COLORTEMP_DIMMABLE_LIGHT ||
